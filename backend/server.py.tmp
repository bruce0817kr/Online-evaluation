...
app = FastAPI(title="?¨ë¼???‰ê? ?œìŠ¤??, version="2.0.0")
...
app.add_middleware(
    ...
    allow_origins=[
        "http://localhost:3000",
        "http://218.38.240.192:3000",  # ?¸ë? IP ?‘ê·¼ ?ˆìš©
...
class Project(BaseModel):
    ...
    description: str
    deadline: datetime
    created_by: str  # secretary ?ëŠ” admin user_id
    created_at: datetime = Field(default_factory=datetime.utcnow)
    is_active: bool = True
...
class ProjectCreate(BaseModel):
    ...
    description: str
    deadline: str
    # created_by???…ë ¥ë°›ì? ?Šê³  ?œë²„?ì„œ current_user.idë¡??ë™ ? ë‹¹
...
class SharePermission(BaseModel): # ?ˆë¡œ??ëª¨ë¸
    user_id: str
...
class EvaluationTemplate(BaseModel):
    ...
    shared_with: List[SharePermission] = [] # ?€??ë³€ê²?ë°?ê¸°ë³¸ê°?[]
...
class ShareRequest(BaseModel): # ?ˆë¡œ??ëª¨ë¸
    user_id: str
    permission: Literal["view", "edit"]

class StatusUpdateRequest(BaseModel): # ?ˆë¡œ??ëª¨ë¸
...
class EvaluationSheet(BaseModel):
    ...
    created_by: Optional[str] = None  # ?‰ê?ë¥??ì„±???¬ìš©??ID
    is_active: bool = True  # ?? œ ?¬ë? ì²´í¬
...
def generate_evaluator_credentials(name: str, phone: str):
    ...
    clean_name = re.sub(r'[^ê°€-?£a-zA-Z]', '', name)
...
async def get_current_user(token: str = Depends(oauth2_scheme)):
    ...
    try:
        ...
        user_id: str = str(user_id_from_payload) # ëª…ì‹œ???•ë???
    except JWTError:
        raise credentials_exception
...
async def get_current_user_optional(token: Optional[str] = Depends(oauth2_scheme)):
    """? íƒ???¬ìš©???¸ì¦ (? í°???ˆìœ¼ë©?ê²€ì¦? ?†ìœ¼ë©?None ë°˜í™˜)"""
    if not token:
        return None
    ...
    try:
        ...
        user_id_from_payload = payload.get("sub")
        if user_id_from_payload is None:
            return None # ? í°??? íš¨?˜ì? ?Šê±°??user_idë¥??¬í•¨?˜ì? ?ŠìŒ
        user_id: str = str(user_id_from_payload) # ëª…ì‹œ???•ë???
            
        user_data = await db.users.find_one({"id": user_id})
        if user_data is None:
            return None # ?¬ìš©?ë? ì°¾ì„ ???†ìŒ
            
        return User(**user_data) # ?¬ìš©???•ì¸ ë°?ê²€ì¦??„ë£Œ
    except JWTError:
        return None # ? í° ?”ì½”???¤íŒ¨

def check_admin_or_secretary(user: User):
    if user.role not in ["admin", "secretary"]:
        raise HTTPException(status_code=403, detail="ê´€ë¦¬ì ?ëŠ” ê°„ì‚¬ë§??‘ê·¼?????ˆìŠµ?ˆë‹¤")
...
async def login(form_data: OAuth2PasswordRequestForm = Depends()):
    ...
    if not user_data or not verify_password(form_data.password, user_data["password_hash"]):
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
            detail="?„ì´???ëŠ” ë¹„ë?ë²ˆí˜¸ê°€ ?˜ëª»?˜ì—ˆ?µë‹ˆ??,
            headers={"WWW-Authenticate": "Bearer"},
        )
...
async def secretary_signup(request: SecretarySignupRequest):
    ...
    if existing_request:
        raise HTTPException(
            status_code=400,
            detail="?´ë? ? ì²­???´ì—­???ˆìŠµ?ˆë‹¤. ê´€ë¦¬ì ?¹ì¸??ê¸°ë‹¤?¤ì£¼?¸ìš”."
        )
    ...
    return {
        "message": "ê°„ì‚¬ ?Œì›ê°€??? ì²­???„ë£Œ?˜ì—ˆ?µë‹ˆ?? ê´€ë¦¬ì ?¹ì¸ ??ë¡œê·¸?¸ì´ ê°€?¥í•©?ˆë‹¤.",
        "request_id": approval_request.id
    }

@api_router.get("/admin/secretary-requests")
async def get_secretary_requests(current_user: User = Depends(get_current_user)):
    """ê°„ì‚¬ ? ì²­ ëª©ë¡ ì¡°íšŒ (ê´€ë¦¬ìë§?"""
    if current_user.role != "admin":
        raise HTTPException(status_code=403, detail="ê´€ë¦¬ìë§??‘ê·¼ ê°€?¥í•©?ˆë‹¤")
...
async def approve_secretary_request(
    ...
    if not request_data:
        raise HTTPException(status_code=404, detail="? ì²­ ?´ì—­??ì°¾ì„ ???†ìŠµ?ˆë‹¤")
    
    if request_data["status"] != "pending":
        raise HTTPException(status_code=400, detail="?´ë? ì²˜ë¦¬??? ì²­?…ë‹ˆ??)
    
    # ?¬ìš©??ê³„ì • ?ì„±
    user_id = str(uuid.uuid4())
    login_id = request_data["name"]  # ?´ë¦„??ë¡œê·¸??IDë¡??¬ìš©
    password_hash = get_password_hash(request_data["phone"].replace("-", ""))  # ?„í™”ë²ˆí˜¸ë¥?ë¹„ë?ë²ˆí˜¸ë¡??¬ìš©
...
async def reject_secretary_request(
    ...
    return {"message": "ê°„ì‚¬ ? ì²­??ê±°ë??˜ì—ˆ?µë‹ˆ??}
...
async def create_admin_user(
    ...
    if existing_user:
        raise HTTPException(status_code=400, detail="?´ë? ì¡´ì¬?˜ëŠ” ?¬ìš©??ID?…ë‹ˆ??)
    ...
    return {
        ...
        "user_name": user_data["user_name"],
        "role": user_data["role"],
        "message": "?¬ìš©?ê? ?±ê³µ?ìœ¼ë¡??ì„±?˜ì—ˆ?µë‹ˆ??
    }
...
async def update_admin_user(
    ...
    return {"message": "?¬ìš©?ê? ?±ê³µ?ìœ¼ë¡??˜ì •?˜ì—ˆ?µë‹ˆ??}
...
async def delete_admin_user(
    ...
    if user_id == current_user.id:
        raise HTTPException(status_code=400, detail="?ê¸° ?ì‹ ?€ ?? œ?????†ìŠµ?ˆë‹¤")
    ...
    return {"message": "?¬ìš©?ê? ?±ê³µ?ìœ¼ë¡??? œ?˜ì—ˆ?µë‹ˆ??}

# ===== ê°„ì‚¬ ë°??‰ê??ìš© API =====
@api_router.get("/secretary/projects")
async def get_secretary_projects(current_user: User = Depends(get_current_user)):
    """ê°„ì‚¬ê°€ ê´€ë¦¬í•˜???„ë¡œ?íŠ¸ ëª©ë¡ ì¡°íšŒ"""
    if current_user.role != "secretary":
        raise HTTPException(status_code=403, detail="ê°„ì‚¬ë§??‘ê·¼ ê°€?¥í•©?ˆë‹¤")
...
async def create_secretary_project(
    ...
    return {"id": project_id, "message": "?„ë¡œ?íŠ¸ê°€ ?±ê³µ?ìœ¼ë¡??ì„±?˜ì—ˆ?µë‹ˆ??}
...
async def update_secretary_project(
    ...
    if result.modified_count == 0:
        raise HTTPException(status_code=404, detail="?„ë¡œ?íŠ¸ë¥?ì°¾ì„ ???†ê±°???˜ì •??ê¶Œí•œ???†ìŠµ?ˆë‹¤")
    
    return {"message": "?„ë¡œ?íŠ¸ê°€ ?±ê³µ?ìœ¼ë¡??˜ì •?˜ì—ˆ?µë‹ˆ??}
...
async def delete_secretary_project(
    ...
    if result.modified_count == 0:
        raise HTTPException(status_code=404, detail="?„ë¡œ?íŠ¸ë¥?ì°¾ì„ ???†ê±°???? œ??ê¶Œí•œ???†ìŠµ?ˆë‹¤")
    
    return {"message": "?„ë¡œ?íŠ¸ê°€ ?±ê³µ?ìœ¼ë¡??? œ?˜ì—ˆ?µë‹ˆ??}

@api_router.get("/secretary/companies")
async def get_secretary_companies(current_user: User = Depends(get_current_user)):
    """ê°„ì‚¬ê°€ ê´€ë¦¬í•˜??ê¸°ì—… ëª©ë¡ ì¡°íšŒ"""
    if current_user.role != "secretary":
        raise HTTPException(status_code=403, detail="ê°„ì‚¬ë§??‘ê·¼ ê°€?¥í•©?ˆë‹¤")
...
async def create_secretary_company(
    ...
    return {"id": company_id, "message": "ê¸°ì—…???±ê³µ?ìœ¼ë¡??ì„±?˜ì—ˆ?µë‹ˆ??}
...
async def update_secretary_company(
    ...
    if result.modified_count == 0:
        raise HTTPException(status_code=404, detail="ê¸°ì—…??ì°¾ì„ ???†ê±°???˜ì •??ê¶Œí•œ???†ìŠµ?ˆë‹¤")
    
    return {"message": "ê¸°ì—… ?•ë³´ê°€ ?±ê³µ?ìœ¼ë¡??˜ì •?˜ì—ˆ?µë‹ˆ??}
...
async def delete_secretary_company(
    ...
    if result.modified_count == 0:
        raise HTTPException(status_code=404, detail="ê¸°ì—…??ì°¾ì„ ???†ê±°???? œ??ê¶Œí•œ???†ìŠµ?ˆë‹¤")
    
    return {"message": "ê¸°ì—…???±ê³µ?ìœ¼ë¡??? œ?˜ì—ˆ?µë‹ˆ??}

@api_router.get("/secretary/evaluations")
async def get_secretary_evaluations(current_user: User = Depends(get_current_user)):
    """ê°„ì‚¬ê°€ ê´€ë¦¬í•˜???‰ê? ëª©ë¡ ì¡°íšŒ"""
    if current_user.role != "secretary":
        raise HTTPException(status_code=403, detail="ê°„ì‚¬ë§??‘ê·¼ ê°€?¥í•©?ˆë‹¤")
...
async def create_secretary_evaluation(
    ...
    return {"id": evaluation_id, "message": "?‰ê?ê°€ ?±ê³µ?ìœ¼ë¡??ì„±?˜ì—ˆ?µë‹ˆ??}
...
async def update_secretary_evaluation(
    ...
    if result.modified_count == 0:
        raise HTTPException(status_code=404, detail="?‰ê?ë¥?ì°¾ì„ ???†ê±°???˜ì •??ê¶Œí•œ???†ìŠµ?ˆë‹¤")
    
    return {"message": "?‰ê? ?•ë³´ê°€ ?±ê³µ?ìœ¼ë¡??˜ì •?˜ì—ˆ?µë‹ˆ??}
...
async def delete_secretary_evaluation(
    ...
    if result.modified_count == 0:
        raise HTTPException(status_code=404, detail="?‰ê?ë¥?ì°¾ì„ ???†ê±°???? œ??ê¶Œí•œ???†ìŠµ?ˆë‹¤")
    
    return {"message": "?‰ê?ê°€ ?±ê³µ?ìœ¼ë¡??? œ?˜ì—ˆ?µë‹ˆ??}

@api_router.get("/evaluator/projects")
async def get_evaluator_projects(current_user: User = Depends(get_current_user)):
    """?‰ê??ê? ì°¸ì—¬ ì¤‘ì¸ ?„ë¡œ?íŠ¸ ëª©ë¡ ì¡°íšŒ"""
    if current_user.role != "evaluator":
        raise HTTPException(status_code=403, detail="?‰ê??ë§Œ ?‘ê·¼ ê°€?¥í•©?ˆë‹¤")
...
async def export_evaluations(
    ...
    if current_user.role not in ["admin", "secretary"]:
        raise HTTPException(status_code=403, detail="ê´€ë¦¬ì ?ëŠ” ê°„ì‚¬ë§??‘ê·¼ ê°€?¥í•©?ˆë‹¤")
    ...
    if template_id:
        ...
        if not template:
            raise HTTPException(status_code=404, detail="?œí”Œë¦¿ì„ ì°¾ì„ ???†ìŠµ?ˆë‹¤")
    
    # ?‰ê???ì¡°íšŒ
    evaluations = await db.evaluation_sheets.find({"project_id": project_id}).to_list(1000)
    if not evaluations:
        raise HTTPException(status_code=404, detail="?‰ê? ê²°ê³¼ê°€ ?†ìŠµ?ˆë‹¤")
    ...
    if format == "excel":
        ...
        raise HTTPException(status_code=400, detail="ì§€?í•˜ì§€ ?ŠëŠ” ?¬ë§·?…ë‹ˆ??)
...
async def create_template(
    ...
    """?ˆë¡œ???‰ê? ?œí”Œë¦??ì„±"""
    check_admin_or_secretary(current_user) # ê¶Œí•œ ?•ì¸

    # project_id???”ì²­ ë³¸ë¬¸???†ìœ¼ë¯€ë¡? ?„ì‹œë¡?"default_project" ?ëŠ” ?¤ë¥¸ ë¡œì§?¼ë¡œ ?¤ì •?´ì•¼ ?©ë‹ˆ??
    # ?¬ê¸°?œëŠ” ?ˆì‹œë¡?"default_project"ë¥??¬ìš©?©ë‹ˆ?? ?¤ì œ êµ¬í˜„ ?œì—???ì ˆ??project_idë¥?ë°›ì•„???©ë‹ˆ??
    # ?ëŠ” EvaluationTemplateCreate ëª¨ë¸??project_idë¥?ì¶”ê??´ì•¼ ?©ë‹ˆ??
    # ???ˆì œ?ì„œ??EvaluationTemplateCreate??project_idê°€ ?†ë‹¤ê³?ê°€?•í•˜ê³?ì§„í–‰?©ë‹ˆ??
    # ?¤ì œ ?„ë¡œ?íŠ¸?ì„œ????ë¶€ë¶„ì„ ë°˜ë“œ???˜ì •?´ì•¼ ?©ë‹ˆ??
    
    # EvaluationItem ê°ì²´ ?ì„±
    items = []
    for item_data in template_data.items:
        # ê°???ª©??project_idë¥?? ë‹¹?´ì•¼ ?˜ì?ë§? ?„ì¬ EvaluationItemCreate?ëŠ” project_id ?„ë“œê°€ ?†ìŠµ?ˆë‹¤.
        # EvaluationItem ëª¨ë¸?ëŠ” project_idê°€ ?„ìˆ˜?…ë‹ˆ??
        # ??ë¶€ë¶„ë„ ?¤ì œ ?„ë¡œ?íŠ¸?ì„œ???˜ì •???„ìš”?©ë‹ˆ?? ?„ì‹œë¡?"default_project" ?¬ìš©
        items.append(EvaluationItem(
            id=str(uuid.uuid4()), # ê°??„ì´?œì—??ê³ ìœ  ID ë¶€??
            name=item_data.name,
            description=item_data.description,
            max_score=item_data.max_score,
            weight=item_data.weight,
            project_id="default_project" # ?„ì‹œê°? ?˜ì • ?„ìš”
        ))
    ...
    new_template = EvaluationTemplate(
        ...
        items=items,
        created_by=current_user.id,
        last_modified=datetime.utcnow() # ?ì„± ??last_modified ?…ë°?´íŠ¸
    )
...
async def get_templates(
    ...
    # ê³µìœ ???œí”Œë¦¿ë„ ?¬í•¨ (ë³¸ì¸???ì„±?ˆê±°?? shared_with???ì‹ ???ˆê±°??
    # ?¬ìš©?ê? ?‘ê·¼?????ˆëŠ” ?œí”Œë¦? ?ì‹ ???ì„±?ˆê±°?? ?ì‹ ?ê²Œ ê³µìœ ???œí”Œë¦?
    # ?ëŠ” ê´€ë¦¬ì/ê°„ì‚¬??ëª¨ë“  ?œí”Œë¦?ì¡°íšŒ ê°€??(? íƒ??
    if current_user.role not in ["admin", "secretary"]:
        query["$or"] = [
            {"created_by": current_user.id},
...
async def get_template(template_id: str, current_user: User = Depends(get_current_user)):
    ...
    if current_user.role not in ["admin", "secretary"] and \
       ...
       not any(s["user_id"] == current_user.id for s in template.get("shared_with", [])):
        raise HTTPException(status_code=403, detail="?œí”Œë¦¿ì— ?‘ê·¼??ê¶Œí•œ???†ìŠµ?ˆë‹¤")
...
async def update_template(
    ...
    if not can_edit:
        raise HTTPException(status_code=403, detail="?œí”Œë¦¿ì„ ?˜ì •??ê¶Œí•œ???†ìŠµ?ˆë‹¤")

    # EvaluationItem ê°ì²´ ?…ë°?´íŠ¸ ?ëŠ” ?ì„±
    updated_items = []
    for item_data in template_data.items:
        # ???ˆì œ?ì„œ??EvaluationItemCreate??IDê°€ ?†ìœ¼ë¯€ë¡? ??ƒ ?ˆë¡œ ë§Œë“ ?¤ê³  ê°€?•í•©?ˆë‹¤.
        # ?¤ì œë¡œëŠ” IDë¥?ê¸°ì??¼ë¡œ ê¸°ì¡´ ?„ì´?œì„ ì°¾ì•„ ?…ë°?´íŠ¸?˜ê±°?? ???„ì´?œì„ ì¶”ê??´ì•¼ ?©ë‹ˆ??
        updated_items.append(EvaluationItem(
            id=str(uuid.uuid4()), # ???„ì´?œìœ¼ë¡?ê°„ì£¼?˜ê³  ??ID ë¶€??
            name=item_data.name,
            description=item_data.description,
            max_score=item_data.max_score,
            weight=item_data.weight,
            project_id=existing_template.project_id # ê¸°ì¡´ ?œí”Œë¦¿ì˜ project_id ?¬ìš©
        ))
    ...
    update_fields = {
        ...
        "version": existing_template.version + 1, # ë²„ì „ ì¦ê?
...
async def delete_template(template_id: str, current_user: User = Depends(get_current_user)):
    ...
    # ?? œ ê¶Œí•œ ?•ì¸ (?ì„±???ëŠ” ê´€ë¦¬ì/ê°„ì‚¬)
    if current_user.role not in ["admin", "secretary"] and template["created_by"] != current_user.id:
        raise HTTPException(status_code=403, detail="?œí”Œë¦¿ì„ ?? œ??ê¶Œí•œ???†ìŠµ?ˆë‹¤")

    # ?¤ì œ ?? œ ?€??is_activeë¥?falseë¡?ë³€ê²½í•˜ê±°ë‚˜ statusë¥?"archived"ë¡?ë³€ê²½í•˜??ê²ƒì„ ê³ ë ¤
    # ?¬ê¸°?œëŠ” statusë¥?"archived"ë¡?ë³€ê²?
    ...
    if result.modified_count == 0:
        raise HTTPException(status_code=404, detail="?œí”Œë¦¿ì„ ì°¾ì„ ???†ê±°???? œ??ê¶Œí•œ???†ìŠµ?ˆë‹¤")
    
    return {"message": "?œí”Œë¦¿ì´ ?±ê³µ?ìœ¼ë¡??? œ?˜ì—ˆ?µë‹ˆ??}
...
async def update_template_status(
    ...
    """?œí”Œë¦??íƒœ ë³€ê²?(draft, active, archived)"""
    ...
    # ?íƒœ ë³€ê²?ê¶Œí•œ ?•ì¸ (?ì„±???ëŠ” ê´€ë¦¬ì/ê°„ì‚¬)
    if current_user.role not in ["admin", "secretary"] and template.created_by != current_user.id:
        raise HTTPException(status_code=403, detail="?œí”Œë¦??íƒœë¥?ë³€ê²½í•  ê¶Œí•œ???†ìŠµ?ˆë‹¤")
    ...
    # draft ?íƒœ??is_activeë¥?ë³€ê²½í•˜ì§€ ?Šê±°?? Trueë¡??¤ì •?????ˆìŒ (?•ì±…???°ë¼)
    # ?¬ê¸°?œëŠ” active???Œë§Œ Trueë¡?ëª…ì‹œ?ìœ¼ë¡??¤ì •
    ...
    updated_template_doc = await db.evaluation_templates.find_one({"id": template_id})
    if not updated_template_doc: # ì¶”ê???None ì²´í¬
        raise HTTPException(status_code=404, detail="?œí”Œë¦??íƒœ ?…ë°?´íŠ¸ ??ë¬¸ì„œë¥?ì°¾ì„ ???†ìŠµ?ˆë‹¤.")
    return EvaluationTemplate(**updated_template_doc)

# ===== ê´€ë¦¬ì/ê°„ì‚¬ ê³µí†µ ?„ë¡œ?íŠ¸ ê´€ë¦?API =====
@api_router.get("/projects")
async def get_projects(current_user: User = Depends(get_current_user)):
    """?„ë¡œ?íŠ¸ ëª©ë¡ ì¡°íšŒ (ê´€ë¦¬ì: ?„ì²´, ê°„ì‚¬: ë³¸ì¸ ?ì„±)"""
    if current_user.role == "admin":
        ...
        raise HTTPException(status_code=403, detail="?„ë¡œ?íŠ¸ ëª©ë¡ ì¡°íšŒ ê¶Œí•œ???†ìŠµ?ˆë‹¤")
    return [
        {
            "id": project.get("id"),
            "name": project.get("name"),
...
async def create_project(
    ...
    """?„ë¡œ?íŠ¸ ?ì„± (ê´€ë¦¬ì/ê°„ì‚¬)"""
    if current_user.role not in ["admin", "secretary"]:
        raise HTTPException(status_code=403, detail="?„ë¡œ?íŠ¸ ?ì„± ê¶Œí•œ???†ìŠµ?ˆë‹¤")
    project_id = str(uuid.uuid4())
    try:
...
async def upload_file(
    ...
    """ê¸°ì—…ë³?PDF/ë¬¸ì„œ ?Œì¼ ?…ë¡œ??""
    # ê¶Œí•œ ì²´í¬: ê´€ë¦¬ì ?ëŠ” ê°„ì‚¬ë§??…ë¡œ??ê°€??
    if current_user.role not in ["admin", "secretary"]:
        raise HTTPException(status_code=403, detail="?Œì¼ ?…ë¡œ??ê¶Œí•œ???†ìŠµ?ˆë‹¤")

    # ê¸°ì—… ì¡´ì¬ ?¬ë? ?•ì¸
    company = await db.companies.find_one({"id": company_id})
    if not company:
        raise HTTPException(status_code=404, detail="ê¸°ì—…??ì°¾ì„ ???†ìŠµ?ˆë‹¤")
    ...
    return {
        ...
        "message": "?Œì¼ ?…ë¡œ?œê? ?„ë£Œ?˜ì—ˆ?µë‹ˆ??
    }
...
async def get_uploaded_file(
    ...
    """?…ë¡œ?œëœ ?Œì¼ ì¡°íšŒ ë°??¤ìš´ë¡œë“œ"""
    # ?Œì¼ ë©”í??°ì´??ì¡°íšŒ
    file_meta = await db.file_metadata.find_one({"id": file_id})
    if not file_meta:
        raise HTTPException(status_code=404, detail="?Œì¼??ì°¾ì„ ???†ìŠµ?ˆë‹¤")

    # ê¶Œí•œ ?•ì¸ (ê´€ë¦¬ì, ê°„ì‚¬, ?´ë‹¹ ê¸°ì—…???‰ê???
    if current_user:
        if current_user.role in ["admin", "secretary"]:
            pass  # ê´€ë¦¬ì/ê°„ì‚¬??ëª¨ë“  ?Œì¼ ?‘ê·¼ ê°€??
        elif current_user.role == "evaluator":
            # ?‰ê??ëŠ” ?ì‹ ???‰ê??˜ëŠ” ê¸°ì—…???Œì¼ë§??‘ê·¼ ê°€??
            evaluation_sheet = await db.evaluation_sheets.find_one({
                "evaluator_id": current_user.id,
                "company_id": file_meta["company_id"]
            })
            if not evaluation_sheet:
                raise HTTPException(status_code=403, detail="?´ë‹¹ ?Œì¼???‘ê·¼??ê¶Œí•œ???†ìŠµ?ˆë‹¤")
        else:
            raise HTTPException(status_code=403, detail="?Œì¼ ?‘ê·¼ ê¶Œí•œ???†ìŠµ?ˆë‹¤")
    else:
    ...
    if not file_path.exists():
        raise HTTPException(status_code=404, detail="?Œì¼??ì¡´ì¬?˜ì? ?ŠìŠµ?ˆë‹¤")
...
async def get_company_files(
    ...
    # ê¶Œí•œ ?•ì¸
    if current_user.role in ["admin", "secretary"]:
        pass  # ê´€ë¦¬ì/ê°„ì‚¬??ëª¨ë“  ê¸°ì—… ?Œì¼ ì¡°íšŒ ê°€??
    elif current_user.role == "evaluator":
        # ?‰ê??ëŠ” ?ì‹ ???‰ê??˜ëŠ” ê¸°ì—…???Œì¼ë§?ì¡°íšŒ ê°€??
        evaluation_sheet = await db.evaluation_sheets.find_one({
            "evaluator_id": current_user.id,
            "company_id": company_id
        })
        if not evaluation_sheet:
            raise HTTPException(status_code=403, detail="?´ë‹¹ ê¸°ì—…???Œì¼???‘ê·¼??ê¶Œí•œ???†ìŠµ?ˆë‹¤")
    else:
        raise HTTPException(status_code=403, detail="?Œì¼ ì¡°íšŒ ê¶Œí•œ???†ìŠµ?ˆë‹¤")
...
async def delete_uploaded_file(
    ...
    """?…ë¡œ?œëœ ?Œì¼ ?? œ"""
    # ê´€ë¦¬ì ?ëŠ” ê°„ì‚¬ë§??Œì¼ ?? œ ê°€??
    if current_user.role not in ["admin", "secretary"]:
        raise HTTPException(status_code=403, detail="?Œì¼ ?? œ ê¶Œí•œ???†ìŠµ?ˆë‹¤")
...
async def get_evaluations(
    ...
    """??• ë³??‰ê? ëª©ë¡ ì¡°íšŒ (?µí•© ?”ë“œ?¬ì¸??"""
    query = {}
    
    # ??• ë³??‘ê·¼ ê¶Œí•œ ?œì–´
    if current_user.role == "admin":
        # ê´€ë¦¬ì: ëª¨ë“  ?‰ê? ì¡°íšŒ ê°€??
        pass
    elif current_user.role == "secretary":
        # ê°„ì‚¬: ?ì‹ ???ì„±???‰ê?ë§?ì¡°íšŒ
        ...
        raise HTTPException(status_code=403, detail="?‰ê? ëª©ë¡ ì¡°íšŒ ê¶Œí•œ???†ìŠµ?ˆë‹¤")
    ...
    return [
        {
            ...
            "is_active": evaluation.get("is_active")  # is_active ?„ë“œ ì¶”ê?
        }
        for evaluation in evaluations
    ]
...
async def create_evaluation(
    ...
    """?ˆë¡œ???‰ê? ?ì„± (ê´€ë¦¬ì/ê°„ì‚¬)"""
    if current_user.role not in ["admin", "secretary"]:
        raise HTTPException(status_code=403, detail="?‰ê? ?ì„± ê¶Œí•œ???†ìŠµ?ˆë‹¤")
...
async def get_evaluation(
    ...
    if not evaluation:
        raise HTTPException(status_code=404, detail="?‰ê?ë¥?ì°¾ì„ ???†ìŠµ?ˆë‹¤")
    ...
    if current_user.role == "admin":
        ...
        if evaluation.get("created_by") != current_user.id:
            raise HTTPException(status_code=403, detail="?´ë‹¹ ?‰ê????‘ê·¼??ê¶Œí•œ???†ìŠµ?ˆë‹¤")
    elif current_user.role == "evaluator":
        # ?‰ê??? ?ì‹ ?ê²Œ ? ë‹¹???‰ê?ë§?ì¡°íšŒ
        if evaluation.get("evaluator_id") != current_user.id:
            raise HTTPException(status_code=403, detail="?´ë‹¹ ?‰ê????‘ê·¼??ê¶Œí•œ???†ìŠµ?ˆë‹¤")
    else:
        raise HTTPException(status_code=403, detail="?‰ê? ì¡°íšŒ ê¶Œí•œ???†ìŠµ?ˆë‹¤")
...
async def update_evaluation(
    ...
    if current_user.role == "admin":
        ...
        if existing_evaluation.get("created_by") != current_user.id:
            raise HTTPException(status_code=403, detail="?´ë‹¹ ?‰ê?ë¥??˜ì •??ê¶Œí•œ???†ìŠµ?ˆë‹¤")
        update_data.update({
            key: value for key, value in evaluation_data.items()
            if key in ["evaluator_id", "company_id", "project_id", "template_id", "status", "deadline", "total_score", "weighted_score"]
        })
    elif current_user.role == "evaluator":
        # ?‰ê??? ?ì‹ ?ê²Œ ? ë‹¹???‰ê????íƒœ?€ ?ìˆ˜ë§??˜ì • ê°€??
        if existing_evaluation.get("evaluator_id") != current_user.id:
            raise HTTPException(status_code=403, detail="?´ë‹¹ ?‰ê?ë¥??˜ì •??ê¶Œí•œ???†ìŠµ?ˆë‹¤")
        # ?‰ê??ëŠ” status, total_score, weighted_scoreë§??˜ì • ê°€??
    ...
    if result.modified_count == 0:
        raise HTTPException(status_code=404, detail="?‰ê? ?˜ì •???¤íŒ¨?ˆìŠµ?ˆë‹¤")
...
async def delete_evaluation(
    ...
    """?‰ê? ?? œ (ê´€ë¦¬ì/ê°„ì‚¬)"""
    if current_user.role not in ["admin", "secretary"]:
        raise HTTPException(status_code=403, detail="?‰ê? ?? œ ê¶Œí•œ???†ìŠµ?ˆë‹¤")
    ...
    # ê°„ì‚¬??ê²½ìš° ?ì‹ ???ì„±???‰ê?ë§??? œ ê°€??
    if current_user.role == "secretary":
        if existing_evaluation.get("created_by") != current_user.id:
            raise HTTPException(status_code=403, detail="?´ë‹¹ ?‰ê?ë¥??? œ??ê¶Œí•œ???†ìŠµ?ˆë‹¤")
    ...
    if result.modified_count == 0:
        raise HTTPException(status_code=404, detail="?‰ê? ?? œ???¤íŒ¨?ˆìŠµ?ˆë‹¤")
    
    return {"message": "?‰ê?ê°€ ?±ê³µ?ìœ¼ë¡??? œ?˜ì—ˆ?µë‹ˆ??}

# ===== ?ˆë¡œ???‰ê? ?œì¶œ ë°?ê²°ê³¼ API =====
...
async def submit_evaluation(
    ...
    try:
        ...
        # ê¶Œí•œ ?•ì¸ (?‰ê???ë³¸ì¸ë§??œì¶œ ê°€??
        if current_user.role == "evaluator" and evaluation.get("evaluator_id") != current_user.id:
            raise HTTPException(status_code=403, detail="?´ë‹¹ ?‰ê?ë¥??œì¶œ??ê¶Œí•œ???†ìŠµ?ˆë‹¤")
        ...
        return {
            "message": "?‰ê?ê°€ ?±ê³µ?ìœ¼ë¡??œì¶œ?˜ì—ˆ?µë‹ˆ??,
        ...
        if isinstance(e, HTTPException):
            raise e
        raise HTTPException(status_code=500, detail=f"?‰ê? ?œì¶œ ì¤??¤ë¥˜ê°€ ë°œìƒ?ˆìŠµ?ˆë‹¤: {str(e)}")
...
async def get_evaluation_results(
    ...
    try:
        ...
        # ê¶Œí•œ ?•ì¸
        if current_user.role == "evaluator" and evaluation.get("evaluator_id") != current_user.id:
            raise HTTPException(status_code=403, detail="?´ë‹¹ ?‰ê? ê²°ê³¼???‘ê·¼??ê¶Œí•œ???†ìŠµ?ˆë‹¤")
        ...
        if isinstance(e, HTTPException):
            raise e
        raise HTTPException(status_code=500, detail=f"?‰ê? ê²°ê³¼ ì¡°íšŒ ì¤??¤ë¥˜ê°€ ë°œìƒ?ˆìŠµ?ˆë‹¤: {str(e)}")
...
async def get_user_evaluations(
    ...
    try:
        ...
        if isinstance(e, HTTPException):
            raise e
        raise HTTPException(status_code=500, detail=f"?¬ìš©???‰ê? ëª©ë¡ ì¡°íšŒ ì¤??¤ë¥˜ê°€ ë°œìƒ?ˆìŠµ?ˆë‹¤: {str(e)}")
...
async def get_evaluation_statistics(
    ...
    """?‰ê? ?µê³„ ì¡°íšŒ"""
    try:
        # ê¶Œí•œ ?•ì¸ (ê´€ë¦¬ì/ê°„ì‚¬ë§??‘ê·¼ ê°€??
        if current_user.role not in ["admin", "secretary"]:
            raise HTTPException(status_code=403, detail="?µê³„ ì¡°íšŒ ê¶Œí•œ???†ìŠµ?ˆë‹¤")
        ...
        if isinstance(e, HTTPException):
            raise e
        raise HTTPException(status_code=500, detail=f"?‰ê? ?µê³„ ì¡°íšŒ ì¤??¤ë¥˜ê°€ ë°œìƒ?ˆìŠµ?ˆë‹¤: {str(e)}")
